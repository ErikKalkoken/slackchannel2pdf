...
class TestLocaleHelper(unittest.TestCase):
    def test_should_init_with_defaults(self):
        # when
        locale_helper = helpers.LocaleHelper()
        # then
    ...
    def test_should_use_given_locale_and_timezone(self):
        ...
        # when
        locale_helper = helpers.LocaleHelper(my_locale=my_locale, my_tz=my_tz)
        # then
    ...
    def test_should_use_locale_and_timezone_from_slack(self):
        ...
        # when
        locale_helper = helpers.LocaleHelper(author_info=author_info)
        # then
    ...
    def test_should_use_fallback_locale_if_none_can_be_determined(self):
        # when
        with patch("slackchannel2pdf.helpers.Locale.default") as mock_default:
    ...
    def test_should_use_fallback_timezone_if_none_can_be_determined(self):
        # when
        with patch("slackchannel2pdf.helpers.get_localzone") as mock_get_localzone:
    ...
    def test_should_format_datetime(self):
        # given
        locale_helper = helpers.LocaleHelper(
            my_locale=babel.Locale.parse("de-DE", sep="-")
        )
        # when
